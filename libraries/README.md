# Libraries
These could be useful as components in a larger projects (possibly with modifications).

- [CGI.hpp](CGI.hpp) -- A Simple Common Gateway Interface (CGI/1.1) script runner library
- [CSV](csv.cpp) -- Typed CSV parser in modern C++
- [FileCache.hpp](FileCache.hpp) -- Lazily caches files in static memory
- [LL.ts](ll.ts) -- Linked List implementation in TypeScript
- [LRUCache.hpp](LRUCache.hpp) -- Templated LRU Cache implementation that allows you to specify internal container types
- [matrix.hpp](matrix.hpp) -- Easy to use math matrix implementation in C++ that abuses templates
- [MentionsExpr](mentions.hpp) -- Mentions expression parser
- [mutilate.cpp](mutilate.cpp) -- append uuid like character sequence to a string
- [vaporwave.js](vaporwave.js) -- make text use vaporwave font
- [WasmNumber.ts](WasmNumber.ts) -- Emulate WebAssembly numeric instructions in JS/TS

## Using
Feel free to use and modify these however you like (everything here is under MIT license btw).
Usually it's good to reference where code came from tho, just a recommendation.
